...
- `submodules/simple-knn/` and `submodules/diff-triangle-rasterization/` contain vendored CUDA extension sources, tracked directly by this repository.
...
conda activate 2DTS
python -m pip install --no-build-isolation --no-deps ./submodules/simple-knn ./submodules/diff-triangle-rasterization
python -m pip install --no-build-isolation --no-deps -e .
python -m pip check
```

These commands create the environment, compile native dependencies, install the editable package, and check dependency consistency. Rebuild extensions after C++/CUDA changes.
