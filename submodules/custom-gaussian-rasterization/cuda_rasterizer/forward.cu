...
// Perform initial steps for each Gaussian prior to rasterization.
template <int C>
__global__ void filter_preprocessCUDA(
	int P, int M,
	const float *orig_points,
	const glm::vec3 *scales,
	const float scale_modifier,
		...
		float *__restrict__ out_color)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			{
				power = -0.5f * pow(con_o.x * d.x * d.x + con_o.z * d.y * d.y + 2 * con_o.y * d.x * d.y, gamma);
			}
			if (power > 0.0f)
				continue;
		...
		float *__restrict__ contrib_max)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			atomicAdd(&contrib_sum[collected_id[j]], contrib);
			atomicMaxFloat(&contrib_max[collected_id[j]], contrib);
...
void FORWARD::filter_preprocess(
	int P, int M,
	const float *means3D,
	const glm::vec3 *scales,
	const float scale_modifier,
